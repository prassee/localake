...
def generate_random_faker_data_per_day(index, start_date, end_date, base_csv_path):
    ...
    for day in range(delta_days):
        ...
        num_records = random.randint(2500, 3500)
        ...
        with open(csv_path, mode="w", newline="") as csvfile:
            ...
            for _ in range(num_records):
                # Random id in [1, 100] so ids repeat across rows and CSV files
...
def generate_upi_events(
    ...
    print(
        f"Generating {total_records:,} events in {num_batches:,} batches → s3://{bucket}/{prefix}/"
    )
